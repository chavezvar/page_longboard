...
public class QualificationGraphDisplay {
	...
	public static String getSampleBarChart(String title, List<QualificationPlatform.Sample> listSamples) {
		...
		String resultValue = "";
		String resultLabel = "";
		resultLabel += "{ \"id\": \"perf\", \"label\": \"Perf\", \"type\": \"string\" }, " + "{ \"id\": \"perfbase\", \"label\": \"ValueBase\", \"type\": \"number\" }," + "{ \"id\": \"perfvalue\", \"label\": \"Value\", \"type\": \"number\" },";

		for (int i = 0; i < listSamples.size(); i++) {
			resultValue += "{ \"c\": [ { \"v\": \"" + listSamples.get(i).label + "\" }," + " { \"v\": " + listSamples.get(i).baseInMs + " }, " + " { \"v\": " + listSamples.get(i).timeInMs + " } " + "] },";
		}
		resultValue = resultValue.substring(0, resultValue.length() - 1);
		resultLabel = resultLabel.substring(0, resultLabel.length() - 1);

		String valueChart = "	{" + "\"type\": \"BarChart\", " + "\"displayed\": true, " + "\"data\": {" + "\"cols\": [" + resultLabel + "], " + "\"rows\": [" + resultValue + "]}, " + "\"options\": { " + "\"bars\": \"horizontal\"," + "\"title\": \"" + title
				+ "\", \"fill\": 20, \"displayExactValues\": true," + "\"vAxis\": { \"title\": \"ms\", \"gridlines\": { \"count\": 100 } }" + "}" + "}";
		// +"\"isStacked\": \"true\","
