...
public class LongboardGraph {
    ...
    public static class ChartParameters
    {
        ...
        public ChartParameters(String titleChart, String typeGraph, String titleCol, String typeCol, String idCol, String titleCol2, String typeCol2,
                String idCol2)
        {
            this.titleChart = titleChart;
            this.typeChart = typeGraph;
    ...
    /**
     * The value of One Column. One column may have MULTIPLE value (then display in red, blue...).
     * 
     * @author pierre-yves
     */
        ...
        for (ChartValuesOnColumn valueGraph : listValue)
        {
            ...
            if (valueGraph.name.size() != chartParameters.typeCol.size() - 1)
                logger.info("Bad number of parameters describe [" + (chartParameters.typeCol.size() - 1) + "] cols in the chartParameters, give ["
                        + valueGraph.name.size() + "] values");
            HashMap<String, Object> onRowData = new HashMap<String, Object>();
            listRows.add(onRowData);

            onRowData.put(
                    "c",
                    getValueOneDimension((count % chartParameters.displayTitleFrequency) == 0 ? valueGraph.titleColumn : "", valueGraph.value.get(0),
                            valueGraph.name.get(0), null, null));
