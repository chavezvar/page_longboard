...
public class JunitCaseHistory {
    ...
    public void test() {
        ...
        try {
            ...
            final HashMap<String, Object> caseDetails = CaseHistory.getCaseDetails(1, false, fis, processAPI, identityAPI, commandAPI);
            ...
            if (synthesis != null)
            {
                ...
                {
                    System.out.println("State[" + oneLine.get("State") + "] name[" + oneLine.get("ActivityName")
                            + "] exe[" + oneLine.get("DateBeginST") + " >> " + oneLine.get("DateEndST")
                            + "] timeEnterConnector[" + oneLine.get("timeEnterConnector")
                            + "] timeUser[" + oneLine.get("timeUser")
