...
public class MonitoringPlatformDetails {
    ...
    public static HashMap<String, Object> getDetails(boolean detailsError, PlatformMonitoringAPI platformMonitoringAPI)
    {
        ...
        /*
         * try {
         * Map<String,GcInfo> gcInfo = platformMonitoringAPI.getLastGcInfo();
         * String gcInfoReturn = "";
         * for (String key : gcInfo.keySet())
         * {
         * gcInfoReturn+=":MemBeforeGc:"+ gcInfo.get(key).getMemoryUsageBeforeGc()
         * + ",MemAfterGc:"+gcInfo.get(key).getMemoryUsageAfterGc()
         * +",Duration:"+gcInfo.get(key).getDuration()
         * +",StartTime:"+simpleDateFormat.format( new Date(gcInfo.get(key).getStartTime()));
         * break; // only one
         * }
         * mapMonitoring.put("LastGCInfo",gcInfoReturn);
         * } catch (Exception e) { mapMonitoring.put("LastGCInfo",detailsError ? "error ["+e.toString()+"]":"Not Implemented"); };
         */
